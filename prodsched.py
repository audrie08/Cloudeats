...
# ---- 
        ...
        # Top KPI metrics row - Updated with Production Plan Performance replacing Spoilage vs Revenue
        ...
        # Second row of KPIs - Updated Man-hr and Manpower with empty string targets
        cols2 = st.columns(6)
        ...
        # Big KPI cards - Updated with Volume and changed OEE to Overall Equipment Effectiveness
...
def prepare_graph_data(df):
    ...
    try:
        ...
        # Convert values to numeric, handling comma-separated values and strings
        def convert_to_numeric(value):
            """Convert value to numeric, handling commas and various formats"""
        ...
        melted_df['Value'] = melted_df['Value'].apply(convert_to_numeric)
...
# --- UPDATED SUBRECIPE COLUMN MAPPINGS ---
SUBRECIPE_COLUMNS = {
    ...
    'machine_start': 9,      # Column J (machines start)
    'machine_end': 24,       # Column Y (machines end)
    'data_start_row': 4,     # Row 5 (0-based)
...
class SubrecipeDataExtractor:
    ...
    def get_machine_names(self):
        """Extract machine names from row 4 (columns J to Y)"""
        machine_names = []
        if len(self.df) > 3:  # Row 4 exists (0-based index 3)
            for col in range(SUBRECIPE_COLUMNS['machine_start'], SUBRECIPE_COLUMNS['machine_end'] + 1):
                if col < len(self.df.columns):
                    machine_name = str(self.df.iloc[3, col]).strip()  # Row 4 (0-based index 3)
                    if machine_name:
                        machine_names.append(machine_name)
        return machine_names
    
    def get_subrecipe_dataframe(self):
        """Extract subrecipe data with machine usage"""
        subrecipe_data = []
        ...
        for row_idx in range(SUBRECIPE_COLUMNS['data_start_row'], SUBRECIPE_COLUMNS['data_end_row'] + 1):
            if row_idx < len(self.df):
                ...
                if item_name:
                    ...
                    for col in range(SUBRECIPE_COLUMNS['machine_start'], SUBRECIPE_COLUMNS['machine_end'] + 1):
                        if col < len(row):
                            usage = safe_value(col)
                            machine_usage.append(usage == "1")  # True if "1", False otherwise
                        else:
                            machine_usage.append(False)
...
def machine_calendar():
    ...
    # Add enhanced CSS styling for calendar view
    st.markdown("""
    <style>
    .subrecipe-header {
        background: #f7d42c;
    ...
    <div class="subrecipe-header">
        ...
        <p><b>Machine Scheduling and Availability Dashboard</b></p>
    </div>
    """, unsafe_allow_html=True)
    ...
    try:
        ...
        for row_idx in range(3, len(df_machines)):
            ...
            if machine_name and machine_name != '' and machine_name.lower() != 'nan':
                ...
                # Extract data from columns C to R (indices 2 to 17)
        ...
        if machine_data:
            ...
            # Add legend matching original colors
            st.markdown("""
            <div class="legend">
                ...
                <div class="legend-item">
                    <div class="legend-color" style="background: linear-gradient(135deg, #dcfce7 0%, #bbf7d0 100%); box-shadow: 0 2px 4px rgba(16, 185, 129, 0.3);"></div>
                    <span>Low Usage</span>
                </div>
                <div class="legend-item">
                    <div class="legend-color" style="background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%); box-shadow: 0 2px 4px rgba(245, 158, 11, 0.3);"></div>
                    <span>Moderate Usage</span>
                </div>
                <div class="legend-item">
                    <div class="legend-color" style="background: linear-gradient(135deg, #dbeafe 0%, #bfdbfe 100%); box-shadow: 0 2px 4px rgba(59, 130, 246, 0.3);"></div>
            ...
            st.markdown('<div class="calendar-header">Machine Availability Calendar</div>', unsafe_allow_html=True)
            
            calendar_html = create_calendar_html(machine_data, headers)
            scrollable_html = f"""
...
def main():
    ...
    main_page_selection = option_menu(
        ...
        styles={
            "container": {
                "max-width": "650px",  # Increased width to accommodate fourth option
                "text-align": "center",
                "border-radius": "20px", 
                "color": "#ffffff",
            },
